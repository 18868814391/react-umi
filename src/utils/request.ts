...
request.use(async(ctx, next) => {
  // 请求前
  const { req } = ctx
  const { options, originUrl, url } = req
  const headers = {}
  ...
  if (options.hostTarget) {
    ctx.req.options.prefix = options.hostTarget
    ctx.req.originUrl = ctx.req.options.prefix + ctx.req.options.url
    ctx.req.url = ctx.req.options.prefix + ctx.req.options.url
  }
